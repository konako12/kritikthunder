  ...
  return (
    <div className={s.root}>
            ...
            {searchOpen && searchResults.length > 0 && (
              <div className={s.searchDrop}>
                {searchResults.map((g) => (
                  ...
                  >
                    {g.cover_url && (
                      <img src={g.cover_url} alt={g.title} className={s.sdImg} />
                    )}
                    <div>
                    ...
                    {g.critic_score !== null && (
                      <div className={`${s.sdScore} ${scoreClass(g.critic_score)}`}>
                        {g.critic_score}
                      </div>
                    )}
                  </Link>
        ...
        <div className={s.hero}>
          ...
          <div className={s.heroBody}>
            ...
            <div className={s.heroBtns}>
              <Link href={`/games/${featured.id}`} className={s.btnPrimary}>
                리뷰 보기
              </Link>
            ...
            <Link href={`/games/${g.id}`} key={g.id} className={s.top5Item}>
              ...
              <div className={s.top5Info}>
                ...
                <div className={s.top5Tags}>
                  {g.platforms.map((p) => <span key={p} className={s.tt}>{p}</span>)}
                  <span className={s.tt}>{g.genre}</span>
                </div>
              </div>
              <div className={s.top5R}>
                <span className={s.t5sl}>전문가</span>
                {g.critic_score !== null ? (
                  <div className={`${s.t5s} ${scoreClass(g.critic_score)}`}>
                    {g.critic_score}
                  </div>
                ) : (
      ...
      <section className={s.section}>
        <div className={s.rowHead}>
          <span className={s.rowTitle}>
            최근 <span className={s.hl}>리뷰된</span> 게임
          </span>
      ...
      <section className={s.section}>
        <div className={s.rowHead}>
          <span className={s.rowTitle}>
            전문가 <span className={s.hl}>최신 리뷰</span>
          </span>
      ...
      {upcoming.length > 0 && (
        <section className={s.section}>
          <div className={s.rowHead}>
            <div>
              <span className={s.rowTitle}>
                출시 예정 <span className={s.hl}>기대작</span>
              </span>
      ...
      <section className={s.section}>
        <div className={s.rowHead}>
          <span className={s.rowTitle}>플랫폼별 탐색</span>
        </div>
